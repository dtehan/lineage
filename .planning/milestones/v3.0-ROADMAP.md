...
**Milestone Goal:** Enable complete column-level lineage capture for SQL queries using wildcard syntax, and deliver visualization improvements for cross-database lineage and graph interaction
...
### ✅ Phase 11: Sort columns alphabetically in lineage graph nodes (Complete 2026-02-19)
...
Plans:
- [x] 11-01-PLAN.md -- Alphabetical column sorting in graph nodes, DetailPanel, and unit tests
...
**Key Decisions (v3.0):**
- WildcardResolver uses in-memory dict cache (not Redis) — single extraction run lifetime, no external dependency
- Batch size 100 tables per DBC.ColumnsJQV query — prevents query explosion in large DBQL workloads
- Confidence score 0.70 for wildcard-expanded lineage (vs 0.95 direct, 0.85 expression) — reflects expansion uncertainty
- Multi-table unqualified SELECT * skipped with warning — ambiguous attribution, fail-safe over guessing
- ELK partitioning + post-layout bounding-box shift required for guaranteed non-overlap (partitioning alone insufficient)
- topoSortDatabases() (Kahn's algorithm) replaces alphabetical — upstream databases placed LEFT per lineage convention
- multiSelectionKeyCode=null when toolbar multi-select active — RF treats every click as selection toggle
- ViewLineageExtractor REPLACE VIEW → CREATE VIEW normalization required — Teradata stores REPLACE VIEW in RequestText

**Technical Debt:**
- Phase 7 ROADMAP showed `[ ]` checkboxes for plans (not updated to `[x]`) — cosmetic inconsistency only, SUMMARY files confirm completion
- View lineage requires re-running populate_lineage.py with --views flag — not automated on schema change
