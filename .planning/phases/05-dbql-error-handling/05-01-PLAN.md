...
must_haves:
  truths:
    - "DBQL extraction uses Python logging module for all output"
    - "ExtractionStats class tracks processed, succeeded, failed, skipped counts"
...
<task type="auto">
  <name>Task 1: Add logging infrastructure to extract_dbql_lineage.py</name>
  <files>database/extract_dbql_lineage.py</files>
  <action>
Add Python logging configuration at module level:
...
3. Do NOT yet replace print() statements - that happens in Task 2 of Plan 02.
  </action>
  <verify>
Run: `cd /Users/Daniel.Tehan/Code/lineage/database && python -c "from extract_dbql_lineage import configure_logging, logger; l = configure_logging(True); print('Logger configured:', l.name)"`
Should print: "Logger configured: dbql_extractor"
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: Add ExtractionStats class for tracking extraction outcomes</name>
  <files>database/extract_dbql_lineage.py</files>
  <action>
...
Note: The existing `self.stats` dict in DBQLLineageExtractor will be replaced by `self.extraction_stats` in Plan 02.
  </action>
  <verify>
Run: `cd /Users/Daniel.Tehan/Code/lineage/database && python -c "from extract_dbql_lineage import ExtractionStats; s = ExtractionStats(); s.record_success(); s.record_failure('123', 'tbl', 'ParseError', 'msg'); s.record_skip(); print(s.summary())"`
Should print: "1 succeeded, 1 failed, 1 skipped"
  </verify>
  <done>
...
2. `ExtractionStats` dataclass exists with record_success/failure/skip/summary methods
3. `check_dbql_access` returns tuple with actionable guidance message
4. Module imports without errors
5. No existing functionality broken (existing print statements still work)
</success_criteria>
