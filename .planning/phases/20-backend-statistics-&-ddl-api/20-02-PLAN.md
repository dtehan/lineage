...
<task type="auto">
  ...
  <verify>
    ...
    Run a Flask route registration check to verify both endpoints are registered and accessible:
    ```bash
    cd /Users/Daniel.Tehan/Code/lineage/lineage-api && python -c "
    import python_server
    app = python_server.app
    rules = {rule.rule: rule.methods for rule in app.url_map.iter_rules()}
    # Check statistics endpoint exists
    stats_route = '/api/v2/openlineage/datasets/<path:dataset_id>/statistics'
    ddl_route = '/api/v2/openlineage/datasets/<path:dataset_id>/ddl'
    assert stats_route in rules, f'Statistics route not found. Routes: {list(rules.keys())}'
    assert 'GET' in rules[stats_route], f'Statistics route missing GET method'
    assert ddl_route in rules, f'DDL route not found. Routes: {list(rules.keys())}'
    assert 'GET' in rules[ddl_route], f'DDL route missing GET method'
    print('Route registration OK: both /statistics and /ddl endpoints registered with GET')
    "
    ```

    Run a Flask test client check to verify endpoints return proper responses (no database needed):
    ```bash
    cd /Users/Daniel.Tehan/Code/lineage/lineage-api && python -c "
    import python_server
    app = python_server.app
    ...
    with app.test_client() as client:
        # Test with a fake dataset ID -- should return 404 or 500 (not a routing error like 405 Method Not Allowed)
        resp_stats = client.get('/api/v2/openlineage/datasets/fake-ns/fake-db.fake-table/statistics')
        resp_ddl = client.get('/api/v2/openlineage/datasets/fake-ns/fake-db.fake-table/ddl')
        # Both should return JSON (not HTML error page), with status 404 or 500
        assert resp_stats.status_code in (404, 500), f'Statistics returned unexpected status {resp_stats.status_code}'
        assert resp_ddl.status_code in (404, 500), f'DDL returned unexpected status {resp_ddl.status_code}'
        assert resp_stats.is_json, 'Statistics response is not JSON'
        assert resp_ddl.is_json, 'DDL response is not JSON'
        data_stats = resp_stats.get_json()
        data_ddl = resp_ddl.get_json()
        assert 'error' in data_stats, f'Statistics response missing error field: {data_stats}'
        assert 'error' in data_ddl, f'DDL response missing error field: {data_ddl}'
        # Verify no SQL/connection details leaked in error messages
        for msg in [data_stats['error'], data_ddl['error']]:
            assert 'DBC' not in msg, f'Error message leaks DBC details: {msg}'
            assert 'SQL' not in msg.upper() or msg == 'Internal server error', f'Error message leaks SQL: {msg}'
        print(f'Test client OK: statistics={resp_stats.status_code}, ddl={resp_ddl.status_code}')
        print(f'Error messages: stats=\"{data_stats[\"error\"]}\", ddl=\"{data_ddl[\"error\"]}\"')
    "
    ```
  ...
  <done>
    ...
    - Flask route registration verified via url_map inspection
    - Flask test client confirms endpoints are routable and return JSON error responses
...
2. Flask url_map inspection confirms both routes registered with GET method
3. Flask test client GET to both endpoints returns 404 or 500 JSON (not 405 or HTML)
4. Grep for `def get_dataset_statistics` in python_server.py confirms function exists
5. Grep for `def get_dataset_ddl` in python_server.py confirms function exists
6. Grep for `"Internal server error"` in both new endpoints (not `str(e)`)
7. Grep for `DBC.TablesV` in python_server.py confirms DBC queries present
8. Grep for `DBC.ColumnsJQV` in python_server.py confirms column comment query
</verification>

<success_criteria>
- Python server imports cleanly with new endpoints
- Both routes registered in Flask url_map and routable via test client
