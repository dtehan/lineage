...
must_haves:
  ...
  key_links:
    ...
    - from: "lineage-api/internal/application/openlineage_service.go"
      to: "lineage-api/internal/adapter/outbound/teradata/openlineage_repo.go"
      via: "s.repo.GetDataset for existence check before statistics/DDL queries"
      pattern: "s\\.repo\\.GetDataset\\(ctx,\\s*datasetID\\)"
...
<task type="auto">
  ...
  <action>
    ...
    **4. Add service methods to openlineage_service.go** (append after existing GetLineageGraph method, before helper methods section):
    ...
    - Call `s.repo.GetDataset(ctx, datasetID)` first to verify dataset exists. If nil, return nil, nil. NOTE: `GetDataset` already exists on the `OpenLineageRepository` interface (see repository.go line 46) and is already implemented in the Teradata repo -- no new method needed for this existence check.
    ...
    - Call `s.repo.GetDataset(ctx, datasetID)` first to verify dataset exists. If nil, return nil, nil. Same note: `GetDataset` is pre-existing.
