...
must_haves:
  ...
  artifacts:
    ...
    - path: "lineage-api/cmd/server/main.go"
      provides: "Decorator wiring -- CachedOpenLineageRepository wraps Teradata repo"
...
<objective>
Implement CachedOpenLineageRepository decorator with cache-aside pattern for GetColumnLineageGraph and GetDataset, wire it into main.go, and write comprehensive unit tests.

Purpose: This is the core caching logic. The decorator wraps the Teradata OpenLineageRepository, checks Redis before querying Teradata, and populates cache on miss. It implements all 15 methods of domain.OpenLineageRepository -- only GetColumnLineageGraph and GetDataset are cached, the other 13 delegate directly to the inner repository. After creating the decorator, this plan also wires it into main.go so the codebase is fully functional.

Output: New cached_openlineage_repo.go with decorator, new cached_openlineage_repo_test.go with tests, updated MockCacheRepository with JSON round-trip support, updated main.go with decorator wiring.
</objective>
...
Run: `cd /Users/Daniel.Tehan/Code/lineage/lineage-api && go build ./internal/adapter/outbound/redis/...` -- must compile to verify the compile-time interface implementation check (`var _ domain.OpenLineageRepository = (*CachedOpenLineageRepository)(nil)`) passes successfully.
  </verify>
  <done>CachedOpenLineageRepository implements domain.OpenLineageRepository. GetColumnLineageGraph and GetDataset use cache-aside. 13 other methods delegate via embedding.</done>
</task>

<task type="auto">
  <name>Task 3: Wire decorator into main.go</name>
  <files>lineage-api/cmd/server/main.go</files>
  <action>
Update `cmd/server/main.go` to wrap the Teradata OpenLineageRepository with the CachedOpenLineageRepository decorator that now exists from Task 2.

1. Remove the blank identifier usage added in Plan 28-01:
   ```go
   // REMOVE this line:
   _ = cacheRepo // Used by CachedOpenLineageRepository in Plan 28-02
   ```

2. Replace the existing OpenLineage repository wiring block. The CURRENT code (from Plan 28-01):
   ```go
...
1. `_ = cacheRepo` line removed
2. `NewCachedOpenLineageRepository(olRepo, cacheRepo, 300)` called
3. `cachedOLRepo` passed to `NewOpenLineageService` (not bare `olRepo`)
4. No changes to service or handler construction
  </verify>
  <done>main.go wraps olRepo with CachedOpenLineageRepository decorator. cachedOLRepo passed to service. Service and handler code unchanged. Project compiles.</done>
</task>

<task type="auto">
  <name>Task 4: Write unit tests for cache decorator</name>
...
11. main.go wraps olRepo with CachedOpenLineageRepository decorator and passes to service
</verification>
...
9. main.go wraps Teradata repo with decorator and passes to service transparently
10. Full project compiles and all existing tests pass
</success_criteria>
