...
4. OpenLineageRepository wiring is UNCHANGED (still `olService := application.NewOpenLineageService(olRepo)`)
5. No changes to service or handler construction
  </verify>
  <done>main.go imports redis adapter, creates Redis client with fail-fast, defers Close. OpenLineage wiring is unchanged. Project compiles.</done>
</task>
...
4. Project compiles successfully -- no broken intermediate state
5. OpenLineage repository wiring untouched (decorator wiring deferred to Plan 28-02)
</success_criteria>
