...
This section references the ops guide for detailed procedures but adds dev-specific information. All cross-references to the operations guide MUST use clickable markdown links with anchors, e.g. `[Operations Guide > Prerequisites](operations_guide.md#prerequisites)`.
...
- **Configuration**: "See [Operations Guide > Configuration](operations_guide.md#configuration) for the full environment variable reference." Add dev-specific notes:
...
- **Database Setup**: "See [Operations Guide > Database Setup](operations_guide.md#database-setup) for QVCI verification, schema creation, and lineage population." Add a brief note about the two population modes: `--fixtures` (default, for demo/testing) and `--dbql` (extracts from query logs).
  ...
  <verify>File exists at docs/developer_manual.md. TOC has all 10 sections. Quick Start has the 7-step setup. Environment Setup contains at least 3 clickable markdown links to operations_guide.md using the format `[link text](operations_guide.md#section)` (e.g., `[Operations Guide > Prerequisites](operations_guide.md#prerequisites)`). Dev-specific notes about Vite proxy, Redis optional, and Playwright port are present.</verify>
  <done>developer_manual.md exists with header, TOC, Quick Start, and Environment Setup sections. Ops guide is referenced via clickable markdown links (not duplicated). Dev-specific details documented.</done>
</task>
...
- Config: `vitest.config.ts` (jsdom environment, setup file at `src/test/setup.ts`) -- both files verified to exist at these paths
- Note: Currently ~558 tests across 32 test files. Some tests may have known failures (accessibility tests). The count changes as tests are added; use "~558" as an approximate.
...
**Closing guidance:** "For day-to-day development, frontend unit tests (`npm test` in watch mode) provide the fastest feedback. Run the full suite before committing."
  ...
  <verify>The Running Tests section has all 5 subsections (Database, API, Frontend Unit, E2E, Go Backend). Each subsection has Command, Requires, and What it validates. Test counts match research-verified numbers: 73 database, 20 API, ~558 frontend unit (verified from research, approximate), 34 E2E. Config paths verified: `vitest.config.ts` exists, `src/test/setup.ts` exists.</verify>
  <done>Running Tests section documents all four test suites plus Go backend tests with commands, requirements, expected output, and what each validates. Test counts are consistent (~558 frontend unit) across the overview table, subsection, and notes.</done>
</task>
