...
must_haves:
  ...
  key_links:
    - from: "docs/developer_manual.md"
      ...
      via: "summarize-and-link pattern"
      pattern: "coding_standards_go\\.md"
    - from: "docs/developer_manual.md"
      to: "specs/coding_standards_typescript.md"
      via: "summarize-and-link pattern"
      pattern: "coding_standards_typescript\\.md"
    - from: "docs/developer_manual.md"
      to: "specs/coding_standards_sql.md"
      via: "summarize-and-link pattern"
      pattern: "coding_standards_sql\\.md"
---
...
- ELKjs for automatic graph layout (hierarchical/layered)
  ...
  <verify>Architecture Overview has Mermaid system diagram. Backend Architecture has hexagonal diagram, directory tree, 5 repository interfaces listed. Frontend Architecture has directory tree, data flow explanation. Both sections reference source files, not abstract descriptions.</verify>
  <done>Architecture Overview, Backend Architecture, and Frontend Architecture sections document system structure with Mermaid diagrams, directory trees, and key patterns.</done>
</task>
...
Link: "Full standards: [specs/coding_standards_sql.md](../specs/coding_standards_sql.md)"
  ...
  <verify>Database & Schema section has Mermaid ER diagram, 9-table reference, lineage traversal explanation. API Reference has v2 endpoint table. Code Standards has summary tables for Go, TypeScript, SQL with links to specs/ files. Each specs/ file is linked.</verify>
  <done>Database & Schema, API Reference, and Code Standards sections complete. OpenLineage schema documented with ER diagram, API endpoints listed, coding conventions summarized with links to canonical specs.</done>
</task>

</tasks>

<verification>
1. Architecture Overview section has Mermaid system diagram
2. Backend Architecture explains hexagonal pattern with diagram, directory tree, 5 repository interfaces
3. Frontend Architecture has directory tree, technology stack, data flow, key patterns
4. Database & Schema has Mermaid ER diagram, 9-table reference, lineage traversal explanation
5. API Reference lists all v2 endpoints with methods and descriptions
6. Code Standards summarizes conventions for Go, TypeScript, and SQL
7. Each code standards subsection links to the canonical specs/ file
8. No content duplicated from operations guide
9. Architecture sections explain "why" patterns exist, not just "what" files do
</verification>
