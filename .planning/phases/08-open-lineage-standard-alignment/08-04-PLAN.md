...
<task type="auto">
  <name>Task 1: Create openlineage_repo.go with namespace and dataset operations</name>
  <files>lineage-api/internal/adapter/outbound/teradata/openlineage_repo.go</files>
  <action>
Create a new file implementing the OpenLineageRepository interface. Start with the struct, constructor, and namespace/dataset operations:
...
```
  ...
  <verify>Run `go build ./...` from lineage-api directory to verify code compiles (will have missing method errors until Task 2 completes).</verify>
  <done>openlineage_repo.go created with struct, constructor, and namespace/dataset CRUD operations.</done>
</task>

<task type="auto">
  <name>Task 2: Add field, job, and run operations</name>
  <files>lineage-api/internal/adapter/outbound/teradata/openlineage_repo.go</files>
  <action>
Append the field, job, and run operations to the existing openlineage_repo.go file:
...
```
  ...
  <verify>Run `go build ./...` from lineage-api directory to verify code compiles (will have missing method errors until Task 3 completes).</verify>
  <done>Field, job, and run CRUD operations added to openlineage_repo.go.</done>
</task>

<task type="auto">
  <name>Task 3: Add lineage traversal methods</name>
  <files>lineage-api/internal/adapter/outbound/teradata/openlineage_repo.go</files>
  <action>
...
<task type="auto">
  <name>Task 4: Add repository tests</name>
