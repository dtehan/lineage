...
<task type="auto">
  ...
  <files>database/populate_lineage.py</files>
  <action>
Add functions to extract and populate OL_* tables. First add the hashlib import at the top of the file with other imports:
...
```
  ...
  <verify>Read the file and verify all 7 functions exist: generate_namespace_id, generate_lineage_id, generate_dataset_id, generate_field_id, populate_openlineage_namespace, populate_openlineage_datasets, populate_openlineage_fields, populate_openlineage_lineage. Also verify `import hashlib` is present.</verify>
  <done>All OpenLineage population functions exist including generate_lineage_id, hashlib is imported, and proper namespace, dataset, field, and lineage handling is in place.</done>
</task>
